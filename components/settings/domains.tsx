...
export default function Domains() {
  ...
  const [{wildcard, userOnly, domain, selectedDomain}, setState] =
    ...
    try {
      ...
      if (data.success) {
        ...
        notification.success({
          message: 'Success',
          description:
            'Added domain successfully, please wait up to 48 hours for it to propagate.',
        });
      }
    } catch (err) {
  ...
  const addRandomDomain = async () => {
    try {
      ...
      domain =
        selectedDomain.wildcard && selectedDomain.subdomain !== ''
      ...
      if (data.success) {
        user = Object.assign({}, user);
        user.settings.randomDomain.domains =
          user.settings.randomDomain.domains.concat([domain]);
  ...
  const deleteRandomDomain = async (domain: string) => {
    try {
      const findDomain = user.settings.randomDomain.domains.find(
        d => d === domain
      );
      ...
      if (data.success) {
        user = Object.assign({}, user);
        user.settings.randomDomain.domains =
          user.settings.randomDomain.domains.filter(d => d !== domain);
  ...
  const columns: any = [
    ...
    },
    {
      ...
      dataIndex: 'wildcard',
      key: 'wildcard',
      render: (wildcard: boolean) => <span>{wildcard ? 'Yes' : 'No'}</span>,
    },
    {
      title: 'Date Added',
      dataIndex: 'dateAdded',
  ...
  return (
    <div>
      {user.premium && (
        <div className={styles.section}>
          ...
          <p className={styles.caption}>
            You can add domains for your use only, or allow others to use them.
          </p>

          <div style={{display: 'flex', flexDirection: 'column'}}>
            <span className="ant-statistic-title" style={{marginTop: -7}}>
              <WarningOutlined /> These settings are not changeable later.
            </span>

            <Checkbox
              defaultChecked={wildcard}
              onChange={val =>
                setState(state => ({...state, wildcard: val.target.checked}))
              }
            >
              Allow Subdomain
            </Checkbox>
            ...
            <Checkbox
              ...
              onChange={val =>
                setState(state => ({...state, userOnly: val.target.checked}))
              }
            >
              Private Domain
            </Checkbox>
      ...
      {user.settings.randomDomain.enabled && (
        <div className={styles.section}>
          ...
          <span className={styles.caption}>
            Your random domain selections are:{' '}
            <span style={{fontWeight: 500}}>
              {user.settings.randomDomain.domains.length <= 0
                ? 'None.'
                : user.settings.randomDomain.domains.map(m => (
          ...
          <Input
            ...
            placeholder={
              selectedDomain.wildcard ? 'Subdomain' : 'Not Available'
            }
      ...
      <div className={styles.section}>
        ...
        <p className={styles.caption}>
          There are currently{' '}
          <strong>{domains.filter(d => d.userOnly === false).length}</strong>{' '}
          public domain(s).
        </p>
